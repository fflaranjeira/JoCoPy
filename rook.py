...
# Joint Count Analysis in a Lattice using Python, rook's case
...
def Ss_Calc (order, a):
    ...
    S0 = (4.0 * pow (order,2)  * 2) +  ( 2 *order * 3 * ( len ( a [0] ) - (2*order) ) ) +  ( 2 *order * 3 * ( len ( a ) - (2*order) ) ) + ( ( len ( a [0] ) - (2*order) ) * ( len ( a ) - (2*order) ) * 4 )
    ...
    S1 = 0.5 * (((4 * pow (order,2) * 2) * ( 2 )**2) + \
             ( 2 *order * ( len ( a [0] ) - (2*order) ) * 3 * ( 2 )**2 ) + \
            ( 2 *order * ( len ( a ) - (2*order) ) * 3 * ( 2 )**2 ) + \
             ( ( len ( a [0] ) - (2*order) ) * ( len ( a ) - (2*order) ) * 4 * ( 2 )**2 ))
    ...
    S2 = 4.0 * (( 4.0 * pow (order,2) * (2)**2) + (2 *order *( len ( a [0] ) - (2*order) ) * (3)**2) + (2 *order *( len ( a ) - (2*order) )  * (3)**2) + ( ( len ( a [0] ) - (2*order) ) * ( len ( a ) - (2*order) )) *(4)**2)
